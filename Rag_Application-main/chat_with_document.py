...
def main():
    ...
    match divice_type:
        ...
        case "GPU":
            llm = LlamaCpp(model_path=model_path, n_ctx=model_n_ctx, n_gpu_layers=90,
                           n_batch=model_n_batch, callbacks=callbacks, verbose=False)
        case _default:
            # raise exception if model_type is not supported
            raise Exception(
                f"Divice type {divice_type} is not supported. Please choose one of the following: CPU , GPU ")
    ...
    qa = RetrievalQA.from_chain_type(llm=llm, chain_type="stuff", retriever=retriever,
                                     return_source_documents=not args.hide_source, chain_type_kwargs=chain_type_kwargs)
